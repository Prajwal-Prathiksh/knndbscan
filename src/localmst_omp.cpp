...
int localmst_omp(const point_int N, const point_int ISTART, const point_int* JA,
                 const float* A, vector<point_int>& R, vector<point_int>& C,
                 const vector<point_int> core, vector<edge_int>& crossE) {
    int rank;
...
#pragma omp parallel for reduction(merge_edgeint : crossE)
        for (point_int c = 0; c < N_core; c++) {
            ...
            while (checked[i] < minPts - 1) {
                ...
                if ((q < 0) or
                    (q >= N)) {  // this a cross edge, need to check the next one
                    crossE.push_back(m);
                    checked[i] += 1;
                    m += 1;
                } else if ((R[q] > -1) and
                           (R[i] != R[q])) {  // this edge is internal edge connecting
                                              // two core points
                    pwrite(minE[R[i]], R[q], A[m]);
                    break;
                } else {  // internal but connecting to a non-core point, need to check
                          // the next one
                    checked[i] += 1;
                    m += 1;
                }
