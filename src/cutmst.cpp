...
void minimum_edges(MPI_Datatype Edgetype, const vector<int> C, const vector<int> C_local, const vector<Edge> minE, map<int, int> &best)
{
    ...
    MPI_Alltoallv(scounts, &n1[0], &s1[0], MPI_INT, rcounts, &n2[0], &s2[0], MPI_INT, MPI_COMM_WORLD);
...
void break_cycles_global(const vector<int> pre_check, vector<int> &roots)
{
    ...
    MPI_Gatherv(&pre_check[0], n_check, MPI_INT, &I1[0], rcounts, rdispls, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gatherv(&roots[0], n_check, MPI_INT, &J1[0], rcounts, rdispls, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Scatterv(&J1[0], rcounts, rdispls, MPI_INT, &roots[0], n_check, MPI_INT, 0, MPI_COMM_WORLD);
...
void output_best(const map<int,int> best, const string name)
{
   ...
   int istart = ilabels[rank];
    ...
    MPI_Gatherv(&sbuf[0], nbuf, MPI_INT, &rbuf[0], rcounts, rdispls, MPI_INT, 0, MPI_COMM_WORLD);
...
vector<int> cutmst(const int n_ALLtrees, int &n_local, vector<edge_int> &crossE, const float *A)
{
    ...
    // Initialization:
        ...
        //C_local: a subset of C with roots onwed my local process := {i\inC: n_start< i < n_end}
    ...
    if(rank == 0) cout<< "input subtree #:"<< n_ALLtrees<<endl;
    while(dn_tree > 0){
    ...
    if(rank ==0) cout<<"number of clusters: "<< n_tree<<endl; 
